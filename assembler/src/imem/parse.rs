...
fn parse_line(line: &str) -> anyhow::Result<Inst> {
    ...
    if lhs.contains("(") {
        let rhs = rhs.trim();
        let lhs = lhs.split(",").map(|e| e.trim()).collect::<Vec<_>>();
        // beq r0, (r0, r0) -> -42
        // beq | r0, (r0, r0) | -42
        // beq | r0, (r0, r0) | -42
        // beq | r0 | (r0 | r0) | -42
    ...
    } else if lhs.starts_with("iret") {
        // lhs: iret 0x1234
        let imm = lhs.trim().split_ascii_whitespace().next().unwrap().trim();
...
fn parse_inst(kind: &str, args: Vec<&str>) -> anyhow::Result<Inst> {
    ...
    let inst_kind = match kind {
        ...
        "xori" => Ok(InstKind::Xori {
            rd: args[0].u8(),
            rs1: args[1].u8(),
            val: args[2].value(),
        }),
        "srli" => Ok(InstKind::Srli {
            rd: args[0].u8(),
            rs1: args[1].u8(),
            val: args[2].value(),
        }),
        "srai" => Ok(InstKind::Srai {
            rd: args[0].u8(),
            rs1: args[1].u8(),
            val: args[2].value(),
        }),
        "slli" => Ok(InstKind::Slli {
            rd: args[0].u8(),
            rs1: args[1].u8(),
            val: args[2].value(),
        }),
